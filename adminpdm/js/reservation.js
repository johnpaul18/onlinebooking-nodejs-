...
const socket = io("https://secasestudy.herokuapp.com");
socket.on("connection");

let currentMessage;

$(function () {
  // $('#date').html(new Date().toLocaleDateString() +'<br>'+ moment().format('hh:mm:ss A'))
...
socket.on("message", (data) => {
  ...
  if (data.id === dataId[currentMessage])
    resevationMessage(dataId.indexOf(data.id), 0);
});
...
const reservations = (pending, dateTitle, tableId, type) => {
  ...
  pending.forEach((data) => {
    ...
    dataSet[0] = `<p><btn
    onclick="resevationMessage(${dataId.indexOf(data._id)},${1})"
    class="
      btn btn-sm btn-outline-success btn-round btn-icon
    "
...
function showpending() {
  ...
  $("#reservedContainer").hide();
  $("#borrowedContainer").hide();
}
function showreserved() {
  $("#pendingContainer").hide();
  $("#reservedContainer").show();
  $("#borrowedContainer").hide();
}
function showborrowed() {
  $("#pendingContainer").hide();
  $("#reservedContainer").hide();
  $("#borrowedContainer").show();
}
function showall() {
  $("#pendingContainer").show();
  $("#reservedContainer").show();
