...
const booksTable = (books) => {
  ...
  books.forEach((data) => {
    ...
    dataSet[4] = `<button
    onclick="updateModalHandler(${JSON.stringify(data)
      .split('"')
      .join("&quot;")})"
    style="
...
const deleteBookHandler = (id) => {
  ...
  demo.showNotification("top", "center", `Deleted`);
  var xhr = new XMLHttpRequest();
...
let updateId;
function updateModalHandler(book) {
  updateId = book._id;
  $("#updateTitle").val(book.title);
  $("#updateAuthor").val(book.author);
  $("#updateCopyrightDate").val(book.copyrightDate);
  $("#updateNumberOfCopies").val(book.numberOfCopies);
...
function updateBookHandler() {
  ...
  var xhr = new XMLHttpRequest();
  xhr.open("PATCH", `${apiUrl}/api/v1/books/${updateId}`, true);
  xhr.setRequestHeader("Content-Type", "application/json");
  xhr.setRequestHeader("Authorization", "Bearer " + user.token);

  xhr.onload = function () {
    demo.showNotification("top", "center", `book updated`);
  };
  ...
  xhr.send(
    JSON.stringify({
      ...
      copyrightDate: $("#updateCopyrightDate").val(),
      numberOfCopies: $("#updateNumberOfCopies").val(),
    })
  );
}
